...
class CalenderHome extends StatefulWidget {
  ...
  int month;
  int year;
  CalenderHome({this.auth,required this.viewOnlyMode,required this.month,required this.year});
  @override
...
class _MyAppState extends State<CalenderHome> {
  ...
  List<String> statusnextday = [];
  List<String> statusprevday = [];
  List<User> allusers = [];
...
//  static FileService fileservice = new FileService();
  ...
  void initState()  {
    ...
    _currentDateTime = DateTime(widget.year, widget.month);
    _selectedDateTime =  DateTime(widget.year, widget.month, date.day);
...
//    getDBState();
    ...
    //define an async anonymous function as can not call async function from initState
    () async {
      statusdays =  await  getBookDates(_currentDateTime);
      _getCalendar();
      setState(() {
  ...
  Future <String> getuserinfo( ) async {
    MatchsResponse resp =   await widget.auth!.getMatchsForMonth(_currentDateTime, Globals.user!.email);
 ...
 //   });
  ...
  Future <List<String>> getBookDates(DateTime? date ) async {
    List <String> statusdays = [];
    BookedDatesResponse resp =   await widget.auth!.GetMonthStatusforUser(date, Globals.user!.email);
    ...
    return ElevatedButton(
        ...
        onPressed:  () async{
          States.clear();
           ...
           UserResponse? resp = await widget.auth?.SetBookedDatesforuser(Globals.user!.email,_currentDateTime,States);
           if (resp!.error == '200')
             {
               AwesomeDialog(
                   context: context,
          ...
          children: [
            Row(
                children: [
                  ...
                  const Text("  Red indicates you are Captain that day  ",
                    style: TextStyle(
                        color: Colors.white),
                  ...
                  const Text("  Green that you have a match",
                    style: TextStyle(
                        color: Colors.white),
                    textAlign: TextAlign.center,),
                ]
 ...
 //     States[calendarDate.date.day] = 0;
    ...
    //now draw the colored button
      return Center(
       child:circleButton(currentColor, 40, 40,Text( '${calendarDate.date!.day}'),
...
Widget circleButton(Color color,width,height,text,VoidCallback performAction) {
  return Container(
